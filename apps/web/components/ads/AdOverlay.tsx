...
/**
 * AdOverlay.tsx — Se auto-completa cuando el countdown termina
 *
 * CAMBIO:
 *   Recibe `adReady` de useAd. Cuando adReady pasa a true,
 *   el botón "Continuar" se habilita. Si el usuario ya estaba
 *   en la pestaña (no fue al vignette), el auto-complete de useAd
 *   llama onContinue automáticamente con 300ms de delay.
 *   Si el usuario estaba en el vignette, el visibilitychange de useAd
 *   lo llama cuando vuelve.
 *
 *   El overlay se cierra porque adMode vuelve a IDLE cuando el servidor
 *   confirma ad-done, lo que hace visible = false.
 */
...
interface Props {
  ...
  skipCount: number;
  threshold: number;
  adReady: boolean;   // ← countdown terminó en useAd
}
...
export default function AdOverlay({ visible, onContinue, skipCount, threshold, adReady }: Props) {
  ...
  useEffect(() => {
    ...
    // Countdown visual (solo UI, la lógica real está en useAd)
    ...
    <>
      <style>{`
        ...
        .adov-root {
          ...
          background: rgba(4,4,12,0.92); backdrop-filter: blur(12px);
        ...
        .adov-skip-row{display:flex;align-items:center;gap:8px;margin-bottom:32px;position:relative;z-index:1;}
        ...
        .adov-message{position:relative;z-index:1;text-align:center;margin-bottom:32px;display:flex;flex-direction:column;align-items:center;gap:10px;}
